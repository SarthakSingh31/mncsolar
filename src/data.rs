use serde_json::{json, Value};

pub const BASE_URL_DEFAULT: &str = "https://mncsolar.com";

pub fn base_url() -> String {
    std::env::var("SITE_BASE_URL").unwrap_or_else(|_| BASE_URL_DEFAULT.to_string())
}

pub const NAV_ITEMS: &[(&str, &str)] = &[
    ("/", "Home"),
    ("/why-solar", "Why Solar"),
    ("/why-mnc-solar", "Why MNC Solar"),
    ("/services", "Services"),
    ("/contact", "Contact"),
];

pub fn nav_items(active: &str) -> Value {
    Value::Array(
        NAV_ITEMS
            .iter()
            .map(|(href, label)| {
                json!({ "href": href, "label": label, "active": *href == active })
            })
            .collect(),
    )
}

pub fn contact() -> Value {
    json!({
        "email": "contact@mncsolar.com",
        "phone": "+91 9643805595",
        "phone_tel": "+919643805595",
        "whatsapp": "919643805595",
        "address": "BMP-16, Rd Number 1, Maurya Vihar Colony, Khagaul, Patna, Bihar 801105",
        "registered_address": "3A, Classics Sarkar Mansion, Circular Road, Ranchi, Jharkhand - 834001"
    })
}

pub fn stats() -> Value {
    json!([
        { "value": "200+", "label": "Installations" },
        { "value": "5+",   "label": "Years Experience" },
        { "value": "₹0",   "label": "Electricity Bill Goal" },
        { "value": "25yr", "label": "Panel Lifespan" }
    ])
}

pub fn services() -> Value {
    json!([
        {
            "id": "ongrid",
            "title": "On-Grid Systems",
            "description": "Grid-connected solar systems that reduce electricity bills while maintaining grid connection.",
            "features": [
                "Net metering support",
                "Grid synchronization",
                "Reduce bills to zero",
                "Government subsidy eligible",
                "High efficiency panels"
            ],
            "icon": "zap",
            "capacity": "2 KW - 100 KW",
            "image": "/images/installations/smart-meter.jpg"
        },
        {
            "id": "offgrid",
            "title": "Off-Grid Systems",
            "description": "Independent solar power systems with battery storage for areas without grid connectivity.",
            "features": [
                "Complete energy independence",
                "Advanced battery storage (LiFePO4)",
                "Remote monitoring",
                "Weather-resistant design",
                "24-hour availability"
            ],
            "icon": "battery",
            "capacity": "2 KW - 50 KW",
            "image": "/images/installations/battery-bank.jpg"
        },
        {
            "id": "hybrid",
            "title": "Hybrid Systems",
            "description": "Best of both worlds — grid connection with battery backup for uninterrupted power.",
            "features": [
                "Grid + battery backup",
                "Power during outages",
                "Ideal for varying loads",
                "Net metering compatible",
                "Smart energy management"
            ],
            "icon": "refresh-cw",
            "capacity": "3 KW - 100 KW",
            "image": "/images/installations/rooftop-array.jpg"
        },
        {
            "id": "institutional",
            "title": "Institutional & Commercial",
            "description": "Custom solar solutions for schools, offices, and commercial establishments with varying load requirements.",
            "features": [
                "Custom load analysis",
                "Mini-grid solutions",
                "No generator dependency",
                "Minimal maintenance",
                "Scalable capacity"
            ],
            "icon": "building-2",
            "capacity": "10 KW - 100 KW",
            "image": "/images/installations/institutional-building.jpg"
        }
    ])
}

pub fn service_areas() -> Value {
    json!(["Jharkhand", "Bihar"])
}
